...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		// 触发 entryOption 这个钩子 -> context 根目录路径, entry 入口
		...
		// 通过HarmonyModulesPlugin的初始化提前注册了多个xxxxDependency对应的xxxx.Template()的映射关系
		...
		// 如果想在内置插件注册完做事情，可以绑 afterPlugins 钩子函数
		compiler.hooks.afterPlugins.call(compiler);
		if (!compiler.inputFileSystem) {
			throw new Error("No input filesystem provided");
		}
		// 一系列解析器的插件注册 均在compiler.resolveFactory.hooks.resolveOptions 时进行执行，在 ResolverFactory.js 的 _create 方法中会进行钩子函数的调用
		...
		// 想在解析器注册完之后做东西，可以绑 afterResolvers 这个钩子
