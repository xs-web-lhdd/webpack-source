...
// tapable 包公开了许多 Hook 类，这些类可用于为插件创建钩子
const {
	SyncHook,
	SyncBailHook,
	AsyncParallelHook,
...
class Compiler {
	...
	constructor(context, options = /** @type {WebpackOptions} */ ({})) {
		this.hooks = Object.freeze({
			...
			// 编译 beforeCompile 之后立即调用，但在一个新的 compilation 创建之前
			compile: new SyncHook(["params"]),
			/** @type {AsyncParallelHook<[Compilation]>} */
			// 构建 compilation 结束之前执行。 异步并行钩子
	...
	emitAssets(compilation, callback) {
		...
		// emitFiles()本质是遍历Compilation.emitAsset()生成的资源文件this.assets[file]，然后从source得到binary (Buffer)内容输出生成文件
		const emitFiles = err => {
...
${other}`);
		...
		// 执行hooks.emit将资源导出到构建目录
		// 写插件时 emit 用的很多,因为它是我们修改输出内容的最后机会
		this.hooks.emit.callAsync(compilation, err => {
			if (err) return callback(err);
			outputPath = compilation.getPath(this.outputPath, {});
			// 先创建输入目录dist,再写入文件
			// mkdirp()是webpack封装的一个方法，本质还是调用fs.mkdir(p, err => {})异步地创建目录，然后调用emitFiles()方法
	...
	createCompilation(params) {
		...
		// 做一个缓存，目的在于当循环编译的时候，不需要再次生成 compilation 对象，减少开销
	...
	newCompilation(params) {
		// 调用 new Compilation 生成 compilation 对象
		const compilation = this.createCompilation(params);
		compilation.name = this.name;
		compilation.records = this.records;
		// 调用 thisCompilation 这个钩子函数（thisCompilation表达的意思是完成 compilation 的实例化）
		this.hooks.thisCompilation.call(compilation, params);
		// 调用 compilation 这个钩子（compilation表达的意思是 编译前的过程做一个调用）
		this.hooks.compilation.call(compilation, params);
		return compilation;
	}

	createNormalModuleFactory() {
		// 清除 _lastNormalModuleFactory 及其对应的缓存...
	 ...
	 */
	compile(callback) {
		// TODO: 这个 params 很关键，在 compile 的整个过程中都会把 params 传进去，很重要！！！
		// 生产一个新的构建对象的参数
		...
		this.hooks.beforeCompile.callAsync(params, err => {
			...
			// TODO: ！！！很重要，这个方法实例化一个 compilation 对象
			const compilation = this.newCompilation(params);

			const logger = compilation.getLogger("webpack.Compiler");

			logger.time("make hook");
			// 执行 compiler 中的 make 钩子,这个钩子很重要!!! TODO:
			this.hooks.make.callAsync(compilation, err => {
				logger.timeEnd("make hook");
				if (err) return callback(err);

				logger.time("finish make hook");
				// 执行 compiler 中的 finishMake 钩子
				this.hooks.finishMake.callAsync(compilation, err => {
					...
					process.nextTick(() => {
						...
						compilation.finish(err => {
							...
							// TODO: 到seal这一步，就是开始将静态资源输出到构建目录了
