...
class NormalModule extends Module {
	...
	_createLoaderContext(resolver, options, compilation, fs, hooks) {
		...
		// 插件中注册的 compilation.hooks.loader 钩子在此刻执行
		// hooks 是上面 static getCompilationHooks 函数的返回值
	 ...
	 */
	// TODO:
	_doBuild(options, compilation, resolver, fs, hooks, callback) { // 这个callback是执行 NormalModule 类中 build 方法时,传递进来的的callback,函数体去build这个函数中去看
		// 拼接上下文参数 loaderContext
...
// 得到的 this._source
// _value: 'import sum from "./sum";\n\nconsole.log("webpack");\nsum(1, 2);\n'
// _valueAsBuffer: undefined
// _valueAsString: 'import sum from "./sum";\n\nconsole.log("webpack");\nsum(1, 2);\n'
// _valueIsBuffer: false
		...
		try {
			// 执行loader之前，调用beforeLoaders这个钩子
			// hooks 是上面 static getCompilationHooks 函数的返回值
		...
		// TODO: 执行loaders 调用 loader-runner 仓库的 runLoaders 转译 module 内容，通常是从各类资源类型转译为 JavaScript 文本
		runLoaders(
			{
				resource: this.resource, // 模块路径
				loaders: this.loaders, // loaders集合
				context: loaderContext, // 上面拼凑的上下文 loaderContext
          ...
          // });
			...
			(err, result) => {
				// 这里的result就是执行loader完之后的结果
				// 例如: result.result.toString() 得到的就是转换后的代码: 'import sum from "./sum";\n\nconsole.log("webpack");\nsum(1,2);\n'
				...
				// TODO: 把执行loader后的结果代码传进入
				// 例如: result.result: 'import sum from "./sum";\n\nconsole.log("webpack");\nsum(1,2);\n'
	...
	/**
	 * @param {WebpackOptions} options webpack options 配置
	 * @param {Compilation} compilation the compilation
	 * @param {ResolverWithOptions} resolver the resolver
	 * @param {InputFileSystem} fs the file system
	 ...
	 */
	// TODO: 开启build模块
	build(options, compilation, resolver, fs, callback) { // 这个callback就是 Compilation 中执行 build 时传进来的 callback
		...
		return this._doBuild(options, compilation, resolver, fs, hooks, err => {
			...
			try {
				// 插件中注册 compilation.hooks.beforeParse 的钩子在此刻执行
			...
			// 检查这个模块是否不应该被解析。如果不应该被解析直接return
			...
			try {
				...
				// 对parse后的AST进行一些系列操作: (import require)依赖收集...
		...
		for (const type of sourceTypes || chunkGraph.getModuleSourceTypes(this)) {
			const source = this.error
				? new RawSource(
					...
					// TODO: 这里其实掉的是 JavaScriptGenerator 中的 generate 方法
				: this.generator.generate(this, {
						dependencyTemplates, // TODO:
						runtimeTemplate,
						moduleGraph,
						chunkGraph,
						runtimeRequirements,
