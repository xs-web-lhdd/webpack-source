...
class JavascriptModulesPlugin {
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				//  对 normalModuleFactory 的 createParser 钩子进行注册
				// 注意执行时机是 normalModuleFactory.hooks.createParser，是在 NormalModuleFactory 的 createParser 中进行的 createParser 的调用
				// 返回的是 JavaScriptParser 的实例
				...
				//  对 normalModuleFactory 的 createGenerator 钩子进行注册
				// 是在 NormalModuleFactory 的 createGenerator 中进行的执行，返回的是 JavaScriptGenerator 的实例
				...
				// 注册 renderManifest 钩子
				// 在 compilation 的 getRenderManifest 方法被调用时触发
	 ...
	 */
	// 非入口entry类型chunk触发代码生成: 异步chunk、符合splitChunkPlugin的cacheGroup规则的chunk
	...
	renderChunk(renderContext, hooks) {
		...
		// 调用Template.renderChunkModules()进行所有module相关代码的生成
		const moduleSources =
    ...
    // 1. 计算出"function __webpack_require__(moduleId)"等代码，包含：
    ...
    //      - "__webpack_require__" 函数
		...
		// 3. 合并 "__webpack_module_cache__ = {}";function __webpack_require_(moduleId) 等加载代码
