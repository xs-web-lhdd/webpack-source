...
class JavascriptParser extends Parser {
	...
	blockPreWalkStatement(statement) {
		...
		switch (statement.type) {
			case "ImportDeclaration": // TODO: import 相关语法的处理:
				this.blockPreWalkImportDeclaration(statement);
				break;
			case "ExportAllDeclaration": // export * 的相关语法处理
				this.blockPreWalkExportAllDeclaration(statement);
				break;
			case "ExportDefaultDeclaration": // export default 相关语法处理
				this.blockPreWalkExportDefaultDeclaration(statement);
				break;
			case "ExportNamedDeclaration": // export 一个对象时的相关语法处理
	...
	blockPreWalkImportDeclaration(statement) {
		// import 导入的内容, 例如: import sum from "./sum"
		// 这里的 source 就是 ./sum
		const source = statement.source.value;
		// 执行 import 钩子
		// import钩子什么时候被注册的呢？在 HarmonyImportDependenciesParserPlugin 中被注册
	 ...
	 */
	/**
	 * source 是要进行解析的代码,有可能会是 AST
	 * state: {
	 * 	compilation: Compilation 的实例
	 * 	module: 模块信息
	 * 	options: webpack配置
	 * 	source: 经过loader执行后的源代码
	 * }
	 */
	parse(source, state) {
		let ast;
		...
		this.statementPath = [];
		this.prevStatement = undefined;
		// TODO: 使用preWalkStatements、blockPreWalkStatements、walkStatements对转化后的AST.body进行分析
