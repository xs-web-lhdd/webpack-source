...
const ruleSetCompiler = new RuleSetCompiler([
	// 初始化了大量的plugin，每一个plugin都进行了ruleSetCompiler.hooks.rule.tap()的监听
...
class NormalModuleFactory extends ModuleFactory {
	...
	constructor({
		...
		super();
		// 看里面hooks的顺序就能看清楚大概（解析module对应的path -> 生成module -> 解析modules -> 生成最终代码）
		this.hooks = Object.freeze({
			...
			// 解析完，如果对应路径是一个module，那么就生成对应module
		...
		// TODO: loader时的rules
		// 传入两个参数,第一个默认的配置参数,第二个是在webpack.config.js中设置的loaders的参数配置
		this.ruleSet = ruleSetCompiler.compile([
			{
				rules: options.defaultRules
			},
			{// 拿到配置中的module中的rules，注意这里面的options的实参是webpack.config.js文件中的module
			 // 可以看Compiler.js文件夹createNormalModuleFactory方法里面的 new NormalModuleFactory
			 // 整个webpack源代码，只new NormalModuleFactory 一次，就是在 Compiler.js 这个文件中
				rules: options.rules
			}
		]);
		...
		// 实例化过程中注册 factorize 和 resolve 这两个钩子
		// TODO:
		this.hooks.factorize.tapAsync(
			...
			(resolveData, callback) => {
				// 调用下面的 this.hooks.resolve.tapAsync 注册的钩子
				this.hooks.resolve.callAsync(resolveData, (err, result) => {
					...
					// 执行 normalModuleFactory.hooks.afterResolve 对应插件注册的的钩子
					this.hooks.afterResolve.callAsync(resolveData, (err, result) => {
						...
						/** createData示例：
context: 'C:\\Users\\LiuHao\\Desktop\\project-debug\\deubg-webpack\\src'
generator: JavascriptGenerator
generatorOptions: undefined
layer: null
loaders: (0) []
matchResource: undefined
parser: JavascriptParser {hooks: {…}, sourceType: 'auto', scope: undefined, state: undefined, comments: undefined, …}
parserOptions: undefined
rawRequest: './src/index.js'
request: 'C:\\Users\\LiuHao\\Desktop\\project-debug\\deubg-webpack\\src\\index.js'
resolveOptions: undefined
resource: 'C:\\Users\\LiuHao\\Desktop\\project-debug\\deubg-webpack\\src\\index.js'
resourceResolveData: {_ResolverCachePluginCacheMiss: true, context: {…}, path: 'C:\\Users\\LiuHao\\Desktop\\project-debug\\deubg-webpack\\src\\index.js', request: undefined, query: '', …}
settings: {type: 'javascript/auto'}
type: 'javascript/auto'
userRequest:'C:\\Users\\LiuHao\\Desktop\\project-debug\\deubg-webpack\
						 */
						const createData = resolveData.createData;

						// 插件中注册的 normalModuleFactory.hooks.createModule 钩子在此处执行
						this.hooks.createModule.callAsync(
							...
							(err, createdModule) => {
								if (!createdModule) {
									...
									if (!createdModule) {
										createdModule = /** @type {Module} */ (
											// TODO: 很关键的异步，创建 NormalModule 模块，createData 如上面的例子所示
											new NormalModule(
								...
								// 插件中注册的 normalModuleFactory.hooks.module 钩子在此处执行
								// 执行 module 这个钩子
								...
								// 这个callback是 this.hooks.factorize 执行时传进去的回调
								// 回调函数的函数体在 NormalModuleFactory 类的 create 方法里面
			...
			(data, callback) => {
				...
				if (!scheme) {
					...
					if (!scheme && !contextScheme) {
						...
						// 判断是不是内联loader
						// 前缀为 "-!"将禁用所有已配置的preLoaders和loaders，但不会禁用postLoaders
						// 前缀为"!"将禁用所有已配置的normal loaders
						// "!!"是忽略webpack.config.js的规则，直接使用"!!"配置的规则
						...
						// 以这个为例： '!!style-loader!css-loader?modules!./styles.css'
						// 截取!!之后的内容，然后通过 !+ 进行分割成数组
						const rawElements = requestWithoutMatchResource
							...
							.split(/!+/);
						// unresolvedResource表示文件的请求链接 它就是资源路径 ./styles.css
						unresolvedResource = rawElements.pop();
						// elements代表解析后的loaders
				...
				const continueCallback = needCalls(2, err => {
					...
					if (
						...
						// 触发this.ruleSet.exec()为普通文件进行loader的筛选
						// 为普通文件筛选出适配的loader列表，比如.scss文件需要sass-loader、css-loader等多个loader的处理
					...
					const continueCallback = needCalls(3, err => {
						...
						// 循环遍历不同的loaders,最终形成最终的allLoaders
						...
						try {
							Object.assign(data.createData, {
								...
								userRequest,
								rawRequest: request,
								loaders: allLoaders, // 根据上面三种类型loaders形成最终的allLoaders
								...
								settings,
								type,
								parser: this.getParser(type, settings.parser), // 拿到 JavaScriptParser 示例
								parserOptions: settings.parser,
								generator: this.getGenerator(type, settings.generator), // 拿到 JavaScriptGenerator 示例
				...
				// 比如: "babel-loader!./index-item.line"
				// 如果整个项目都不使用内联模式的loader
				// 那么resolveRequestArray传入的参数就是一个空数组！
				this.resolveRequestArray(
					contextInfo,
					contextScheme ? this.context : context,
					elements, // 上面解析出来的loader列表
					loaderResolver,
					resolveContext,
					(err, result) => {
				...
				const defaultResolve = context => {
					...
					// resource without scheme and with path
						...
						// 根据不同type获取对应的resolver对象
						// 两种类型resolver，一种是处理loader类型的loaderResolver，一种是处理普通文件请求的loaderResolver
	 ...
	 * @returns {void}
	 */
	// 被 Compilation 中的  _factorizeModule 函数里面的 factory.create 调用
	create(data, callback) {
		...
		this.hooks.beforeResolve.callAsync(resolveData, (err, result) => {
			...
			// 执行 constructor 中定义的 factorize 这个钩子, 会创建一个 module,并将module传给回调函数
			this.hooks.factorize.callAsync(resolveData, (err, module) => {
				...
				// 这个callback是create函数被调用时传进来的callback
				// 真正的 callback 函数体是 Compilation 中 _factorizeModule 方法 中 factory.create 里面的第二个参数
	...
	// 解析到路径
	// 对待不同类型的请求，比如文件类型、目录类型、模块类型在resolver.resolve()有不同的执行流程
	// 这个由 enhanced-resolve 库实现，本质上就是替换别称，寻找文件所在目录路径，拼凑成最终的绝对路径
	resolveResource(
		contextInfo,
		context,
		unresolvedResource,
...
If changing the source code is not an option there is also a resolve options called 'preferRelative' which tries to resolve these kind of requests in the current directory too.`
	...
	createParser(type, parserOptions = {}) {
		...
		// 拿到 JavaScriptParser 的实例
		// 为什么能拿到这个 JavaScript 的实例？因为在 JavaScriptModulesPlugins中在注册了normalModuleFactory.hooks.createParser的钩子，返回值是 JavaScriptParser 的实例
		const parser = this.hooks.createParser.for(type).call(parserOptions);
		if (!parser) {
			throw new Error(`No parser registered for ${type}`);
		}
		// 执行 NormalModuleFactory的parser钩子
