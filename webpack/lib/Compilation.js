...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		// params中参数是这两个: 是在 Compiler 中 new Compilation 中传递进来的
		...
		this.assets = {}; // 存放资源 key 是文件名 value 是文件的代码
	...
	_addModule(module, callback) {
		...
		this._modulesCache.get(identifier, null, (err, cacheModule) => {
			...
			// 这个callback是 Compilation.js 中 handleModuleCreation 方法中 this.factorizeModule 中第二个参数中 this.addModule 中的第二个参数
	 ...
	 * @returns {void}
	 */
	_buildModule(module, callback) { // 这个 callback 是执行 buildModule 时传进去的第二个参数
		const currentProfile = this.profile
			? this.moduleGraph.getProfile(module)
			: undefined;
		if (currentProfile !== undefined) {
		...
		module.needBuild(
			...
			(err, needBuild) => {
				...
				// TODO: 进行build, 这个module是 NormalModuleFactory 在 hook.createModule 时通过 new NormalModule 创建的 module
				// 所以可以去 NormalModule 中去查看 build 这个方法
				module.build(
					this.options,
					this,
					this.resolverFactory.get("normal", module.resolveOptions), // 拿到resolver
					this.inputFileSystem, // 读
					err => {
						if (currentProfile !== undefined) {
							currentProfile.markBuildingEnd();
						}
						...
						this._modulesCache.store(module.identifier(), null, module, err => {
							...
							// 这个callback是执行 this.buildModule 时传进去的回调函数
							return callback();
						});
					}
				);
...
结束最外层的handleModuleCreation()回调，结束make流程
		 ...
		 */
		/**
			建立module与dependency之间的关联（this.moduleGraph=new ModuleGraph()）
		 ...
		 * @returns {void}
		 */
		// processDependencyForResolving()主要是进行resourceIdent的筛选以及同一个request的dependency的合并
		...
		try {
			...
			do {
				...
				// import依赖 模块的依赖模块,对依赖列表进行循环然后处理依赖
				if (block.dependencies) {
					currentBlock = block;
					let i = 0;
		...
		// 全部处理完成调用 onDependenciesSorted, 这个方法很重要 TODO:
		if (--inProgressSorting === 0) onDependenciesSorted();
	}
	 ...
	 * @returns {void}
	 */
	// TODO: 根据文件类型构建 module 子类
	handleModuleCreation(
		{
			factory, // normalModuleFactory
			dependencies, // 是当前module的依赖
			originModule, // 当前的module
			contextInfo,
			context,
			recursive = true,
		...
		this.factorizeModule(
			...
			(err, factoryResult) => {
				...
				// 拿到新创建的模块,这个模块是在 NormalModuleFactory 的 hooks.createModule 时通过 new NormalModule 创建的
				...
				// 把newModule加入到 addModuleQueue 模块队列里面
				this.addModule(newModule, (err, module) => {
					...
					this._handleModuleBuildAndDependencies(
						...
						module,
						recursive,
						callback // 这个 callback 是 handleModuleCreation 方法被调用时传进去的 callback
					);
				});
			}
		);
	...
	_factorizeModule(
		...
		// 调用的是 NormalModuleFactory 的 create 方法
		factory.create(
			{
				contextInfo: {
			...
			(err, result) => {
				...
				// 这个 callback 是 Compilation 的 handleModuleCreation 方法中的 this.factorizeModule 被调用时传进去的 callback
				// 真正的 callback 函数体是 handleModuleCreation 方法中的 this.factorizeModule 的第二个参数
	...
	addModuleTree({ context, dependency, contextInfo }, callback) {
		...
		//  这个是： normalModuleFactory
		// 这个 dependencyFactories 是在 EntryOptionPlugin 里面的 EntryPlugin 中在 compilation 时设置的，设置的值是 normalModuleFactorty
		...
		this.handleModuleCreation(
			...
			(err, result) => {
				if (err && this.bail) {
					...
					callback(); // 这个 callback 是调用 addModuleTree 函数时的第二个参数
				}
			}
		);
	}
	...
	/**
	 * @param {string} context context path for entry 根目录
	 * @param {Dependency} entry entry dependency that should be followed 入口模块的相对路径
	 * @param {string | EntryOptions} optionsOrName options or deprecated name of entry 入口的名字
	 * @param {ModuleCallback} callback callback function 编译完成的回调(最终的回调)
	...
	_addEntryItem(context, entry, target, options, callback) {
		...
		this.addModuleTree(
			...
			(err, module) => {
				...
				return callback(null, module); // 这个 callback 是调用 _addEntryItem 时传进来的
			}
		);
	}
	 ...
	 */
	// TODO: 把代码块封装成 chunk
	// seal 函数主要完成从 module 到 chunks 的转化
	seal(callback) {
		...
		// 遍历 this.entries 创建 Chunk 和 ChunkGroup
		// 一开始遍历this.entries实际就是遍历入口文件,其中name是入口文件的名称,dependencies就是入口文件类型的EntryDependency
		// 比如: entry: {A: ["1.js", "2.js"]}，ChunkA存在1.js和2.js，此时的this.entries.dependencies就是1.js和2.js
		for (const [name, { dependencies, includeDependencies, options }] of this
			.entries) {
			// 根据name进行addChunk()创建一个新的chunk
			// 入口chunk
			const chunk = this.addChunk(name);
			if (options.filename) {
				chunk.filenameTemplate = options.filename;
			}
			// 根据 options 创建对应的 new Entrypoint() 也就是 chunkGroup
			// EntryPoint是入口类型的ChunkGroup
			const entrypoint = new Entrypoint(options);
			 // 多个Map对象的设置
			if (!options.dependOn && !options.runtime) {
				entrypoint.setRuntimeChunk(chunk); // 后面生成runtime代码有用
			}
			// 给entrypoint设置 __entrypointChunk = chunk
			entrypoint.setEntrypointChunk(chunk);
			// 进行一系列对象的存储：namedChunkGroups、entrypoints、chunkGroups，为后续的逻辑做准备
			this.namedChunkGroups.set(name, entrypoint);
			this.entrypoints.set(name, entrypoint);
			this.chunkGroups.push(entrypoint);
			// 进行chunk和ChunkGroup的关联
      ...
      // };
			...
			for (const dep of [...this.globalEntry.dependencies, ...dependencies]) {
				...
				// 通过dep获取对应的NormalModule，即利用dependency获取对应的Module对象
				// 入口module
				const module = this.moduleGraph.getModule(dep);
				if (module) {
					// 使用chunkGraph.connectChunkAndEntryModule()关联chunk、module和chunkGroup的关系
					chunkGraph.connectChunkAndEntryModule(chunk, module, entrypoint);
					entryModules.add(module);
					const modulesList = chunkGraphInit.get(entrypoint);
					if (modulesList === undefined) {
						// 进行chunkGraphInit数据结构的初始化，使用entrypoint作为key，将对应入口所包含的Module都加入到数组中
						// 比如entry: {A: ["1.js", "2.js"]}，ChunkA存在1.js和2.js，此时的this.entries.dependencies就是1.js和2.js，chunkGraphInit根据entrypoint创建的数组包含1.js和2.js
			...
			// assignDepths()方法会遍历入口module所有的依赖，为每一个module设置深度标记
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
		...
		// chunk 集合构建完毕后触发 optimizeChunks 钩子
		// 调用hooks.optimizeChunks.call()进行chunks的优化，会触发多个Plugin执行
		// 其中有我们最熟悉的就是SplitChunksPlugin插件
		while (this.hooks.optimizeChunks.call(this.chunks, this.chunkGroups)) {
			/* empty */
		}
		this.hooks.afterOptimizeChunks.call(this.chunks, this.chunkGroups);

		// 调用 optimizeTree 钩子函数，此时的 chunks 和 modules 是经过优化后的 chunks 及 modules 集合
	...
	// 遍历 modules 数组，完成所有module代码转化，并将结果存储到compilation.codeGenerationResults中
	 ...
	 */
	// 以module为基础单位,进行module的代码生成,包括module的同步依赖、异步依赖和对应的runtimes
		...
		cache.get((err, cachedResult) => {
			...
			if (!cachedResult) {
				try {
					...
					// TODO: NormalModule 的 codeGeneration 方法
	...
	processRuntimeRequirements({
		...
		// 遍历所有 module，收集所有 module 的 runtime 依赖
		for (const module of modules) {
		...
		// 遍历所有 chunk，将 chunk 下所有 module 的 runtime 统一收录到 chunk 中
		...
		// 遍历所有 runtime chunk，收集其对应的子 chunk 下所有 runtime 依赖
		// 之后遍历所有依赖并发布 runtimeRequirementInTree 钩子
		// (主要是) RuntimePlugin 插件订阅该钩子并根据依赖类型创建对应的 RuntimeModule 子类实例
...
This prevents using hashes of each other and should be avoided.`);
		...
		const processChunk = chunk => {
			...
			for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
				// 运行时代码会走到这里来,运行时没有hash,业务代码有hash
	 ...
	 * @returns {void}
	 */
	// emitAsset 标记 chunk 的输出规则，即转化为 assets 集合
	// 文件名 源代码
	...
	getRenderManifest(options) {
		// 触发 renderManifest 钩子， 获取render()对象，此时还没生成代码
	 ...
	 * @returns {void}
	 */
	// 合并runtime代码（包括立即执行函数，多种工具函数）、modules代码、其它chunk相关的桥接代码 ，并调用 emitAsset()输出产物
	// 创建chunkAssets资源
	createChunkAssets(callback) {
		...
		asyncLib.forEachLimit(
			...
			(chunk, callback) => {
				...
				// 遍历manifest，调用 (fileManifest=manifest[i]) fileManifest.render()
				asyncLib.forEach(
					manifest,
