...
const applyWebpackOptionsBaseDefaults = options => {
	// 如果 options 的 context 属性是 undefined, 那么把 process.cwd() 的值赋给 options 的 context 属性
	F(options, "context", () => process.cwd());
	// 对 options 的 infrastructureLogging 进行一系列赋值操作
...
const applyOptimizationDefaults = (
	...
	// 初始化阶段就定义了两个默认的cacheGroups配置，其中一个是node_modules的配置
	...
	if (splitChunks) {
		...
		F(cacheGroups, "defaultVendors", () => ({
			idHint: "vendors",
			reuseExistingChunk: true,
			test: NODE_MODULES_REGEXP, // node_modules相关会打包形成一个chunk
			priority: -10
		}));
	}
};
...
const applyInfrastructureLoggingDefaults = infrastructureLogging => {
	// infrastructureLogging 的 stream 属性是 process.stderr
	F(infrastructureLogging, "stream", () => process.stderr);
	// 判断是不是在终端
	const tty =
		/** @type {any} */ (infrastructureLogging.stream).isTTY &&
		process.env.TERM !== "dumb";
	// 给 infrastructureLogging 的 level 赋值为 info
	// debug 为 false
	// colors 是否是终端
