...
class WebpackCLI implements IWebpackCLI {
  ...
  async makeCommand(
    ...
    // 基于 commandOptions 对 command 进行一些列定义(command, description alias usage dependencies)
  ...
  async run(args: Parameters<WebpackCLICommand["parseOptions"]>[0], parseOptions: ParseOptions) {
    ...
    this.program.action(async (options, program: WebpackCLICommand) => {
      ...
      // 如果是内部定义的command，执行loadCommandByName这个函数
      if (isKnownCommand(commandToRun)) {
        ...
        if (isEntrySyntax) {
          ...
          if (found) {
            this.logger.error(
              `Did you mean '${getCommandName(found.name)}' (alias '${Array.isArray(found.alias) ? found.alias.join(", ") : found.alias
              }')?`,
            );
          }
  ...
  async buildConfig(
    ...
    const internalBuildConfig = (item: WebpackConfiguration) => {
        ...
        if (problems) {
          ...
          for (const path in problemsByPath) {
            ...
            for (const problem of problems) {
              this.logger.error(
                `${this.capitalizeFirstLetter(problem.type.replace(/-/g, " "))}${problem.value ? ` '${problem.value}'` : ""
      ...
      if (
        ...
        this.logger.warn(
          `No need to use the '${options.argv.env["WEBPACK_WATCH"] ? "watch" : "serve"
